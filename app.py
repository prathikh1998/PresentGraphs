...
app = Flask(__name__)
cache = redis.Redis(host='quizredis.redis.cache.windows.net', port=6379, password='ynkp3itVINJCqSXZDygXgqoo1baX48GwDTAzCaM4ZFX0=', ssl=True)
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            # Check if the result is cached
                ...
                # Cache the results
                    cache_results(query, str(rows))
                else:
                # Use the cached results
                    query_results.append((query, query_time, eval(result)))
